...
def on_text(event):
    try:
        handle_incoming(event.reply_token, event.source.user_id, event.message.text.strip())
    except Exception as e:
        print(f"[LINE Bot錯誤] on_text: {type(e).__name__}: {e}")
        reply(event.reply_token, "抱歉，系統剛剛忙線了一下，可以再說一次嗎？")
...
def on_audio(event):
    try:
        ...
        if not text:
            reply(event.reply_token, "抱歉，我剛剛沒有聽清楚，可以再說一次，或用打字的也可以：")
            return
        handle_incoming(event.reply_token, line_user_id, text)
    except Exception as e:
        print(f"[LINE Bot錯誤] on_audio: {type(e).__name__}: {e}")
        reply(event.reply_token, "抱歉，語音辨識剛剛出了點問題，可以用打字的試試看嗎？")


# ── 核心對話邏輯（依 state 分流，取代網頁版的分頁畫面）──
def handle_incoming(reply_token: str, line_user_id: str, user_text: str):
    ...
    if state == "awaiting_password":
        ...
        if employee and bcrypt.checkpw(user_text.encode(), employee["password_hash"].encode()):
            ...
            reply(reply_token, "帳號或密碼不正確，請重新輸入你的員工帳號：")
        return
    ...
    if state == "awaiting_mode":
        if "深度" in user_text:
            ...
            reply(reply_token, "請點選下面其中一個模式喔：", quick_items=mode_quick_reply())
            return

        material = get_company_training_material(session["company_id"])
        questions_pool = material.get("questions", [])
        if not questions_pool:
            reply(reply_token, "目前你們公司還沒有上傳訓練教材，麻煩先請主管到 PitchCoach 後台上傳，稍後再回來練習喔！")
            return
    ...
    if state == "in_conversation":
        ...
        if is_complete:
            ...
            reply(reply_token, report_messages[:4], quick_items=mode_quick_reply())
        else:
            save_session(line_user_id, {
                "chat_history": chat_history,
                "current_q_idx": new_idx,
            })
            reply(reply_token, clean_reply)
        return

    # 防呆：狀態異常時重置回綁定流程
    save_session(line_user_id, {"state": "awaiting_username"})
    reply(reply_token, "咦，好像哪裡卡住了，我們重新開始吧！請輸入你的員工帳號：")
